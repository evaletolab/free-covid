...
## Que propose-t-il ?
**«Nous pouvons mettre fin à la pandémie AUJOURD'HUI!»** Selon le Dr Mina, tous les outils sont disponibles pour contrôler la propagation du virus. Le problème c'est la méthode inédite, elle ne trouve pas de place dans un univers où les bureaucrates et les experts se renvoient la balle du périmètre et des responsabilités. Mais le Dr Mina n'abandonne pas, avec une formation de base d'ingénieur, le Dr Mina a un certain goût, c'est même une admiration, à la résolution de problèmes pratiques. «C'est avec les connaissances validées par les scientifiques que les ingénieurs rendent possible ce qui était alors impossible». Malheureusement une solution technique hors norme pour résoudre un problème sanitaire dans l'urgence, a de la peine à convaincre. Même les journalistes ont peur de suivre une route qui pourrait se retourner contre eux. Pour la plupart, la meilleure décision, c'est d'attendre et de voir qui va bouger le premier.
...
Les tests antigéniques rapides bon marchés et sans prescription, augmentent massivement la détection des cas positifs. Ils permettent d'activer beaucoup plus rapidement l'isolement des personnes contagieuses, ce qui entraîne un effondrement de la chaîne de contagion et fait baisser mécaniquement le taux de reproduction R0. [La recherche a montrée](https://www.nejm.org/doi/full/10.1056/NEJMp2025631) qu'une utilisation fréquente des tests rapides et bon marchés diminue significativement le nombre d'infections. 
...
* La solution antigénique testée à San-Fransisco avec un résultat très positif,  [BinaxNOW](https://abbott.mediaroom.com/2020-12-16-Abbotts-BinaxNOW-COVID-19-Rapid-Test-Receives-FDA-Emergency-Use-Authorization-for-First-Virtually-Guided-At-Home-Rapid-Test-Using-eMeds-Digital-Health-Platform)
...
Un dépistage efficace couplé avec des mesures sanitaires simples, participent au maintient du taux de reproduction `R0` le plus bas possible. La solution de dépistage utilisée en Suisse comme en Europe consiste à, dès les premiers symptômes, contacter son médecin, l’hôpital ou toutes offres alternatives légales. A Genève il existe plusieurs solutions de tests rapides comme par exemple avec m3-test.ch. Ce sont des solutions de désengorgement des centres médicaux, mais toutes nécessite une prescription ou un rendez-vous et  sans symptôme ils coûtent chers. En décembre 2020 la confédération a décidé d'élargir l'utilisation des tests rapides antigéniques, mais malheureusement ces tests doivent être effectués par un professionnel formé ([[src]](https://www.bag.admin.ch/bag/fr/home/krankheiten/ausbrueche-epidemien-pandemien/aktuelle-ausbrueche-epidemien/novel-cov/testen.html#2051828467)). 
Pour les curieux, on trouve une très bonne présentation [[de simulation d'une épidémie et de l'intérêt du taux R0]](https://www.youtube.com/watch?v=gxAaO2rsdIs&ab_channel=3Blue1Brown), elle est réalisée par Grant Sanderson.
...
Le Dr Mani, propose une solution simple qui a de bonnes chances de reprendre le contrôle de la propagation et ainsi permet d'éviter le solution du Bazooka. Il faut suivre la zone contagieuse du virus avec la plus petite latence possible. En déployant des tests rapides, bon marchés (*<= 5 fr*) et sans prescription, on prend le contrôle la reproduction du virus.
* **0)** les mesures barrières (masque, hygiène et distance), 
* **1)** le dépistage (SwissCovid et tests), **⇐ c'est ici qu'on peut agir autrement**
* **2)** l'isolation, 
* **3)** le Lockdown _(bombe atomique a éviter)_. 

Comme indiquer plus haut, la solution de l'équipe du Pr Michael Mina, consiste à renforcer les mesures de dépistage.  **Le dépistage est un problème technique qui peut être amélioré avec des ordres de grandeurs d'un facteur 100**
...
Tous les lieux considérés comme clusters potentiels; spectacles, restaurant, bistrots, commerces, clubs, etc,  pourraient être ouverts si et seulement si, on est certains que les personnes contagieuses n'y entre pas. Avec des tests rapides, et en option une application mobile on peut approcher une certitude à presque 100%.
